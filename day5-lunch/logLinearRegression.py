...
mod = smf.ols(formula="logadd1FPKM ~ H3k27ac_tab + H3k27me3_tab + H3k4me1_tab + H3k4me3_tab + H3k9ac_tab", data=small_df)
results = mod.fit()
print(results.summary())
# print(results.resid)
minimum = np.min(results.resid)
maximum = np.max(results.resid)
# print(minimum) this = -3.90027
# print(maximum) this = 7.9703
#
fig,ax = plt.subplots()
ax.set_title("Linear regression plot of residuals log(1+FPKM)")
hist = ax.hist(results.resid, bins=400, range=[-20,20])
ax.set_xlabel("Residual values")
ax.set_ylabel("Counts")
fig.savefig("LogresidualPlot.png")
plt.close()